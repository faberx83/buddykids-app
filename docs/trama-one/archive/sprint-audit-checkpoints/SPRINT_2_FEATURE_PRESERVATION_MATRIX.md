...
| Lettura per Parent/booking | **Funzione dati pronta, MA non ancora collegata a nessuna UI Parent — correzione del 2026-07-21, vedi `ASSUMPTION_LOG.md` V2 e `SPRINT_3_FEATURE_PRESERVATION_MATRIX.md`** | `lib/data/activity-days.ts::getActivityDays()` esiste ed è chiamabile, ma verificato via grep: zero riferimenti in `DetailClient.tsx`/`SearchClient.tsx`/`SearchDiscoveryClient.tsx`/`BookingClient.tsx`. Oggi è usata solo lato Gestore. Collegarla a una UI Parent è lavoro di Build Sprint 3, non una capability già presente |
