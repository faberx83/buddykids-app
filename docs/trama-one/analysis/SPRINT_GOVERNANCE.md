...
#### Backlog vincolante di Sprint 6 (aggiunto in sede di Evidence Patch, 29/07/2026 — vedi `AUDIT_CHECKPOINT_INTEGRATION_SPRINT_1_4.md` §16)

Questi quattro item sono **obbligatori** nello scope di Sprint 6, non opzionali: emersi durante l'Integration Gate (Gate E `CORE_DOMAIN_SOURCE_OF_TRUTH.md` e Gate C/F triage) come rischi noti e circoscritti, mai bloccanti per la chiusura Sprint 1-4 ma esplicitamente rimandati a qui.

- **P1 — Capacity a tripla fonte di verità.** `activities.spots_left` (editoriale), `activity_weeks.spots_left`, `activity_days.spots_left` sono tre campi indipendenti, scollegati, decrementati da percorsi di codice diversi (vedi `CORE_DOMAIN_SOURCE_OF_TRUTH.md` §5). Da fare in Sprint 6: definire un servizio canonico di capacità con invarianti esplicite (mai negativa, mai superiore al totale posti), semantica reservation/release unificata (non tre `UPDATE` sparsi), e test dedicati che verifichino l'invariante sotto concorrenza (doppia accettazione simultanea, cancellazione+accettazione in corsa).
- **P1 — Feature flag override expiry silenziosa.** L'override globale `TRAMA_ONE_ENABLED` è scaduto il 22/07/2026 senza che nessuno se ne accorgesse, disattivando silenziosamente TRAMA ONE in produzione per una settimana (vedi Gate C, ottava ondata, `GATE_C_TRIAGE_20260728.md`). Da fare in Sprint 6: visibilità Admin (una vista che elenca tutti gli override attivi/scaduti/in scadenza per `feature_flag_overrides`), un alert (email o pannello) quando un override è scaduto o scade entro N giorni, un evento di telemetria dedicato quando `resolveFeatureFlag()` restituisce `false` per assenza/scadenza di un override che risultava attivo di recente (prevenzione del fallback silenzioso).
- **P2 — Notifiche email Partner fire-and-forget.** Nessuno stato di consegna persistito per le email di accettazione/rifiuto Partner (Gap P0 #360, `CORE_DOMAIN_SOURCE_OF_TRUTH.md` §8) — se l'invio fallisce silenziosamente, non c'è modo di saperlo dal DB. Da fare in Sprint 6: stato di consegna minimo (tabella o colonna `email_delivery_status`), logging esplicito su fallimento, retry minimo (anche solo un secondo tentativo automatico prima di arrendersi).
- **P2 — TC-N414/N415, persistenza Walkthrough Partner.** Il test di persistenza del primo step del walkthrough "Pubblica la tua prima attività" (`tests/one/walkthrough-partner.spec.ts`) ha fallito in modo simile per due run consecutivi nonostante la serializzazione già applicata (Gate C, nona/decima ondata) — il candidato più solido a bug reale residuo di questo intero ciclo di stabilizzazione. Da fare in Sprint 6: investigare `tutorial_progress`/`app/actions` del walkthrough con evidenza diretta (`error-context.md`/trace del prossimo fallimento), non ipotesi.
