...
**Aggiornamento 2026-07-21 (DEC-38)** — prima fetta di implementazione chiusa: selezione giorni nel dettaglio, calcolo costo dinamico, prenotazione a Giorni spot (`booking_days`, migration_12, NON ancora applicata in produzione) sono passate da MANCANTE a IMPLEMENTATA. Restano MANCANTI, onestamente non silenziati: selezione servizi per giorno (e di conseguenza nessun costo navetta su Giorni spot), context object unico (source/week/child/filters/correlationId — resta PARZIALE, solo query param sciolti), filtro di RICERCA per disponibilità giornaliera (oggi si può scegliere il giorno solo dopo aver aperto il dettaglio, non filtrare `/search`/`/nextgen/search` per esso), estensione dell'avviso "settimana già impegnata" (`findWeekOverlapConflicts`) al caso giorni.
