...
> **Aggiornamento (DEC-73)**: quarto giro di riscontro Fabrizio, testando il deploy di DEC-72 — 4 problemi trovati/corretti. (1) **Cutout ancora "squadrato"**: nonostante DEC-70 leggesse già il border-radius reale del target, il ring lo applicava SENZA tener conto che il cutout è il target ingrandito di 8px per lato — un raggio piccolo (es. 6px di "Salva modifiche") diventa quasi impercettibile su un box più grande. Corretto: il raggio del ring ora è raggio_reale + 8px, mantenendo la stessa curvatura visiva anche sul box ingrandito. Verifica con particolare attenzione la **riga 10/15** (angoli del cutout attorno a "Salva modifiche"/"+ Nuova attività", ora visibilmente arrotondati, non più squadrati). (2) **Flash nella transizione tra step**: per una frazione di secondo, passando da uno step all'altro, popover/cutout potevano restare ancorati alla posizione del target APPENA CONCLUSO mentre già mostravano il testo del nuovo step — corretto azzerando subito la posizione nota alla transizione, invece di aspettare la prossima misurazione. Effetto atteso ora: nessuna sovrapposizione visibile durante i passaggi 2/5/6/7 sotto (può sparire per un istante e ricomparire corretto, mai mostrare la posizione sbagliata). (3) **Badge "dashboard" senza indizio su mobile**: a cassetto laterale chiuso, il badge "target non trovato" per l'ultimo step ora include anche una nota — "Su schermi piccoli, apri il menu ☰ in alto per trovare la voce 'Dashboard' nel cassetto laterale" — verifica **riga 17** a 390px con il cassetto chiuso. (4) **Sottotitolo "Preferenze"**: lato Gestore (`/center/account`) il sottotitolo della voce "Preferenze" nel menu Impostazioni ora recita "Lingua, tema, notifiche, tour guidato" (non più solo "Lingua, tema, notifiche") — non correlato allo Spotlight in sé, ma alla scopribilità del bottone di riavvio tour introdotto in DEC-70.
...
7. Prova a cliccare un giorno del calendario: il tour NON deve avanzare da solo. Clicca "Ho finito, continua →" — il tour avanza a "Monitora dalla dashboard" (`dashboard`, già avviato da solo), l'ultimo step, che resta invece click-based. L'overlay compare ora sulla voce **"Dashboard"** nel menu laterale, su QUALSIASI pagina `/center/*` (nav item persistente). Controlla **riga 17** — su mobile, a cassetto CHIUSO, deve comparire il badge "target non trovato" con la nota "Su schermi piccoli, apri il menu ☰ in alto per trovare la voce 'Dashboard' nel cassetto laterale" (DEC-73); apri il cassetto e verifica che l'evidenziazione compaia normalmente sulla voce "Dashboard".
...
**Aggiornamento (DEC-73)**: quinto giro di riscontro di Fabrizio, 4 problemi trovati/corretti — (1) il ring del cutout, pur leggendo già il raggio reale del target (DEC-70), non teneva conto che il cutout è quel rettangolo ingrandito di 8px per lato: un raggio piccolo restava quasi impercettibile sul box più grande ("ancora squadrato"), ora il raggio del ring è raggio_reale + 8px; (2) nella transizione automatica tra step (DEC-72) poteva comparire per un singolo frame popover/cutout ancorati alla posizione del target appena concluso mentre mostravano già il testo del nuovo step — corretto azzerando subito la posizione nota al cambio di step; (3) il badge "target non trovato" per lo step `dashboard` ora include, a cassetto mobile chiuso, una nota su come renderlo visibile ("apri il menu ☰..."); (4) il sottotitolo della voce "Preferenze" nel menu Impostazioni Gestore ora menziona anche il tour guidato (riga 21 sotto).

| # | URL / account | Check | Mobile | Tablet | Desktop | Note |
|---|---|---|---|---|---|---|
| 10 | `/center/activities` — `center_admin`, step `create_activity` in corso | Overlay scuro con cutout ben allineato attorno al pulsante "+ Nuova attività", stessa forma/raggio d'angolo del pulsante reale (raggio reale + 8px di padding, non un raggio fisso né lo stesso raggio grezzo del pulsante — DEC-70/DEC-73), popover leggibile con titolo "Crea l'attività", "Passo 1 di 6", pulsante "Inizia" (SOLO qui, il primissimo step — DEC-72) | | | | |
...
| 15 | `/center/activities/[id]`, step `publish` | Overlay ancorato al pulsante "Salva modifiche" (step invariato: click-based, nessun "Inizia" né "Ho finito", già avviato — DEC-72), angoli del cutout visibilmente arrotondati (non squadrati — DEC-73); nel PASSAGGIO verso questo step da "Servizi extra e pasto" non deve comparire per un istante un popover/cutout sovrapposto/ancorato altrove (DEC-73, segnalato da Fabrizio come "sovrapposto" nel passaggio step 3→4) | | | | |
...
| 17 | Qualunque pagina `/center/*`, step `dashboard` | Overlay ancorato alla voce "Dashboard" nel menu — su desktop/tablet (≥768px) è già visibile nella sidebar; **su mobile (390px) apri prima il menu (☰) in alto a sinistra**: solo con il cassetto aperto il nav item è visibile e viene evidenziato (DEC-70 — con il cassetto chiuso compare invece il badge "target non trovato" della riga 18, comportamento corretto, non un bug); a cassetto chiuso il badge deve ora includere la nota "Su schermi piccoli, apri il menu ☰ in alto per trovare la voce 'Dashboard' nel cassetto laterale" (DEC-73 — prima di questo fix il badge compariva ma senza alcun indizio su come procedere) | | | | |
...
| 21 | `/center/account/preferenze` — `center_admin` | Nuova sezione "Tour guidato" con pulsante "Riavvia il tour guidato" (DEC-70) — visibile solo se il flag TRAMA_ONE_ENABLED è attivo per l'account; cliccandolo il messaggio cambia in "Percorso riavviato..." e una successiva visita a `/center/activities` riparte dallo step 1. Nel menu Impostazioni di `/center/account` (prima di entrare in Preferenze), il sottotitolo della voce "Preferenze" deve leggere "Lingua, tema, notifiche, tour guidato" (DEC-73) — lato Genitore (`/profile`) il sottotitolo resta invariato ("Lingua, tema, notifiche", nessuna menzione: lo Spotlight Parent non esiste ancora) | | | | |
