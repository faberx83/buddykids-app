...
| 16 | `/center/activities/[id]` (dopo aver salvato), step `configure_spot_days` — target reale su un'altra pagina | Badge "target non trovato" con titolo "Configura i Giorni spot" E un link cliccabile **"Vai al Calendario disponibilità →"** che porta a `/center/activities/[id]/calendar`; una volta lì, overlay ancorato al calendario disponibilità, non alla card KPI/occupazione sopra di esso | | | | |
...
| 18 | Navigare a una pagina senza il target dello step corrente e diverso da "Giorni spot" (es. atterrare su `/center/profile` con lo step ancora "Informazioni di base"/"Servizi extra e pasto") | Badge "target non trovato" in basso a destra (titolo + descrizione dello step), SENZA alcun link (il link compare solo per lo step Giorni spot, riga 16) — non un overlay vuoto o un errore silenzioso | | | | |
