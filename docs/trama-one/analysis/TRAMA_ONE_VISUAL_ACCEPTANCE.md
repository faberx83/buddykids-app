...
3. Vai (o torna) su `/center/activities` e apri **un'attività già esistente** dalla lista (il centro di test ne ha già 2, non serve crearne una nuova) — atterri su `/center/activities/[id]`. Vedi l'overlay sulla card **"Informazioni generali"** (nome, fascia d'età, prezzo a settimana, descrizione), popover con pulsante "Inizia". Controlla **riga 13**.
4. Clicca "Inizia" — il popover mostra ora il testo "Compila con calma i campi di questa sezione, poi continua." e un pulsante **"Ho finito, continua →"** (DEC-71). Prova a cliccare/compilare un campo reale della card (es. il prezzo a settimana): il tour NON deve avanzare da solo. Solo cliccando esplicitamente "Ho finito, continua →" il tour avanza a "Servizi extra e pasto" (`configure_pricing`), l'overlay si sposta sulla card omonima nella stessa pagina (ingresso anticipato, uscita posticipata, opzione pasto). Controlla **riga 14**.
5. Ripeti lo stesso pattern: clicca "Inizia", prova a cliccare un campo reale (es. la checkbox "Ingresso anticipato (pre-servizio)") e verifica che il tour NON avanzi, poi clicca "Ho finito, continua →" — il tour avanza a "Pubblica" (`publish`, questo step resta invece click-based: l'azione stessa di salvare è la conferma genuina). Resti sulla stessa pagina di modifica. Vedi l'overlay sul pulsante **"Salva modifiche"** in fondo alla form. Controlla **riga 15**.
6. Clicca il pulsante "Salva modifiche" — il tour avanza a "Configura i Giorni spot" (`configure_spot_days`). **Il target di questo step è su un'altra pagina** (`/center/activities/[id]/calendar`): siccome resti sulla pagina di modifica dopo il salvataggio, ora deve comparire il badge "target non trovato" in alto a destra CON un link cliccabile **"Vai al Calendario disponibilità →"** — verifica che il link ci sia e sia cliccabile prima di navigare tu stesso. Controlla **riga 16**. Clicca il link (o naviga manualmente): vedi l'overlay sul riquadro del calendario disponibilità, popover con pulsante "Inizia".
7. Clicca "Inizia" — anche qui (DEC-71) prova a cliccare un giorno del calendario: il tour NON deve avanzare da solo. Clicca "Ho finito, continua →" — il tour avanza a "Monitora dalla dashboard" (`dashboard`), l'ultimo step, che resta invece click-based. L'overlay compare ora sulla voce **"Dashboard"** nel menu laterale, su QUALSIASI pagina `/center/*` (nav item persistente). Controlla **riga 17** — su mobile verifica che il menu (drawer/hamburger) esponga comunque questa voce in modo raggiungibile.
...
| 13 | `/center/activities/[id]` (scheda di modifica di un'attività esistente), step `configure_weeks` | Overlay ancorato alla card "Informazioni generali" (nome, fascia d'età, prezzo a settimana, descrizione — titolo popover "Informazioni di base"), nessuna sovrapposizione con altri elementi della form; dopo "Inizia", il popover mostra "Compila con calma i campi di questa sezione, poi continua." + pulsante "Ho finito, continua →" (DEC-71) | | | | |
| 14 | `/center/activities/[id]`, step `configure_pricing` | Overlay ancorato alla card "Servizi extra e pasto" (ingresso anticipato, uscita posticipata, opzione pasto); dopo "Inizia", stesso pulsante "Ho finito, continua →" del check precedente (DEC-71) | | | | |
| 15 | `/center/activities/[id]`, step `publish` | Overlay ancorato al pulsante "Salva modifiche" (step invariato: click-based, nessun pulsante "Ho finito") | | | | |
| 16 | `/center/activities/[id]` (dopo aver salvato), step `configure_spot_days` — target reale su un'altra pagina | Badge "target non trovato" (in alto a destra, sotto l'header — DEC-70) con titolo "Configura i Giorni spot" E un link cliccabile **"Vai al Calendario disponibilità →"** che porta a `/center/activities/[id]/calendar`; una volta lì, overlay ancorato al calendario disponibilità (non alla card KPI/occupazione sopra di esso), dopo "Inizia" stesso pulsante "Ho finito, continua →" (DEC-71) | | | | |
...
| 22 | Allo step `configure_weeks` o `configure_pricing` (dopo "Inizia"), cliccare un campo REALE dentro la card (es. la checkbox "Ingresso anticipato (pre-servizio)" in "Servizi extra e pasto") (DEC-71) | Il click sul campo NON fa avanzare il tour: il popover resta sullo stesso step con "Ho finito, continua →" visibile; puoi continuare a compilare gli altri campi senza interruzioni; il tour avanza SOLO cliccando esplicitamente quel pulsante | | | | |
