...
**Deliberatamente NON agganciati in questo primo giro** (priorità IMPORTANT/aggregato nel sistema in-app, o eventi computed senza un singolo punto di mutazione): `group_request_accepted`, `center_checkins_unconfirmed` (nessun "evento" singolo — è un aggregato ricalcolato), i due match carpool (richiederebbero ri-eseguire la logica di matching dentro `upsertCarpoolOfferAction`/`upsertCarpoolRequestAction` per trovare l'ALTRO utente da notificare — rimandato, non un'omissione silenziosa).
...
### Estensione trigger P0: inquiry (31/08/2026, stesso giorno)

Segnalato da Fabrizio dopo un test reale con due account (`faberx83+testparent`/`faberx83+partnernew`): una richiesta "Contatta il gestore" e la relativa risposta del centro non generavano alcuna push, perché `inquiry_reply`/`center_inquiry_new` erano stati lasciati fuori dal primo giro (elenco sopra). Rivalutato: per un ticketing "a un solo giro" (nessuna chat multi-messaggio, vedi `activity_inquiries`) sia la domanda del genitore sia la risposta del centro SONO l'unica azione attesa su quella richiesta — quindi ACTION a tutti gli effetti, non IMPORTANT. Aggiunto in `app/actions/inquiries.ts`: `createInquiryAction` → push a tutti gli admin del centro dell'attività (stesso pattern "tutti gli admin" di `sendGroupRequestAction`/`createBookingAction`); `replyToInquiryAction` → push al genitore con il testo della risposta nel body, deep link `/richieste`. Stesso principio best-effort, nessuna nuova tabella.

### Bug trovato con lo stesso test: toggle "acceso" senza subscription reale (31/08/2026)

Dopo il trigger inquiry sopra, Fabrizio ha rifatto il test con gli stessi due account: ancora nessuna push. Verifica read-only via MCP Supabase su `push_subscriptions` per i due `user_id`: **zero righe** per entrambi, nonostante `profiles.notify_push = true` per entrambi — quindi `sendPushToUser` era sempre un no-op silenzioso (nessun subscriber a cui inviare, non un errore: comportamento by-design, vedi commento in `lib/push/send.ts`).

Causa: `ProfilePreferencesSection.tsx` inizializzava il checkbox "Notifiche push" direttamente da `profiles.notifyPush` (vero per entrambi gli account, valore ereditato da quando il toggle era ancora cosmetico, prima che esistesse una vera subscription). Il checkbox appariva quindi già "acceso" al caricamento — nessun motivo di ricliccarlo, quindi `enablePushNotifications()` (l'unica funzione che chiede il permesso al browser e scrive su `push_subscriptions`) non partiva mai.

Fix: nuova `hasBrowserPushSubscription()` in `lib/push/client.ts` (verifica reale lato browser via `pushManager.getSubscription()`, nessuna query DB). `ProfilePreferencesSection` la chiama al mount e, se `notifyPush` è true ma QUESTO browser non ha una subscription attiva, corregge SOLO la visualizzazione (mai una scrittura automatica su `profiles.notifyPush`, perché la persona potrebbe avere una subscription vera su un ALTRO device — `notifyPush` è un booleano per profilo, `push_subscriptions` è per-device). L'utente vede quindi il toggle davvero spento e ricliccandolo ottiene una vera iscrizione.
