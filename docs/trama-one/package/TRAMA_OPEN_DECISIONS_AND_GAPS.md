...
**As-of timestamp**: 2026-08-05T20:10:00Z (UTC) — aggiornato con OD-02 formalizzato come decisione A/B, OD-11 sharpened, OD-12 nuovo (gap di crosswalk), OD-13 nuovo (A-MVP-05)
...
| OD-02 | Selezione multipla del Calendario disponibilità (Gestore) non applica il campo "Giornata particolare" (es. "tutti i venerdì piscina") — solo `isOpen`/capacità/sconto/last-minute sono nel `BulkDraft` | Medio-alto (feature richiesta esplicitamente incompleta) | Claude (fix applicativo, solo dopo decisione) | Fix + test Playwright + verifica statica | **Bug confermato**. Root cause: `BulkDraft` (`components/AvailabilityCalendar.tsx`) non include `specialEmoji`/nota. **CONFLICT registrato nel QA Remediation (05/08 sera)**: `PT-MVP-08` era classificato `LIVE` nel Master Requirement Catalog v1 mentre questo stesso OD-02 lo descrive P0/incompleto — incoerenza riconosciuta e corretta: `PT-MVP-08` è ora `CONFLICT`, non `LIVE`, finché non arriva la decisione sotto. **Decisione richiesta a Fabrizio** — due opzioni, non decidibili da Claude: **A. FIX BEFORE BETA** (PT-MVP-08 resta PARTIAL finché il fix non è fatto e verificato) oppure **B. ACCEPTED DEFER** ("Giornata particolare" bulk esplicitamente rimossa dagli acceptance criteria della Beta, pianificata post-Beta; PT-MVP-08 torna LIVE con scope ridotto dichiarato). | **Aperto — CONFLICT, decisione A/B richiesta a Fabrizio prima che lo stato di PT-MVP-08 possa essere chiuso** |
