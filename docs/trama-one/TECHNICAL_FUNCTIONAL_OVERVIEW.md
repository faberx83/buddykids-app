...
- **`curated_favorites`** (migration_38, POST-DISCOVERY CONSOLIDATION 23/09/2026 — **NON ANCORA APPLICATA**) — Preferiti su Scoperta TRAMA: `parent_id`, `curated_lead_id` (testo, id del dataset code-based `lib/discovery/real-dataset.ts`, MAI un uuid/FK verso `activities`), `promoted_to_activity_id` (nullable, valorizzato manualmente quando un Curated Lead diventa un Partner reale — la risoluzione avviene a lettura, in `lib/discovery/unified-favorites.ts`, mai con una scrittura automatica in `favorites`). Stessa RLS di `favorites` (genitore proprietario + bypass admin fin da subito, non come fix successivo).
- **`announcement_receipts`** (migration_38, POST-DISCOVERY CONSOLIDATION 23/09/2026 — **NON ANCORA APPLICATA**) — stato di interazione per Novità TRAMA/Feature Announcements: `parent_id`, `announcement_id` + `announcement_version` (riferimento testuale al catalogo code-based `lib/announcements/catalog.ts`, mai una FK), `seen_at` (Level 1 — bell del Notification Center), `dismissed_at` (Level 2 — callout contestuale "✨ Nuovo"). Il contenuto editoriale (titolo/corpo/deep link/audience) resta SEMPRE nel catalogo code-based, mai duplicato in questa tabella — stesso principio già seguito da `lib/releases/catalog.ts`.
...
10. **Preferiti unificati** (POST-DISCOVERY CONSOLIDATION, 23/09/2026): `getUnifiedFavoritesForParent()` (`lib/data/favorites.ts`) combina i Preferiti Partner (`favorites`) e i Preferiti Curated (`curated_favorites`, migration_38 — **NON ANCORA APPLICATA**) in un'unica lista (`components/PreferitiView.tsx`, mai due sezioni separate). Il cuore è disponibile anche su `DiscoveryLeadCard`/`DiscoveryMapPopupCard` (Scopri, lista e Mappa) con lo stesso pattern optimistic-UI + rollback del cuore Partner (`ActivityCard.tsx`). Transizione Curated → Partner: `curated_favorites.promoted_to_activity_id`, risolto a lettura in `lib/discovery/unified-favorites.ts` (nessuna scrittura automatica in `favorites`, nessun duplicato, nessun re-click richiesto al genitore).
11. **Novità TRAMA / Feature Announcements** (POST-DISCOVERY CONSOLIDATION, 23/09/2026): catalogo editoriale code-based `lib/announcements/catalog.ts` (stesso principio di `lib/releases/catalog.ts` — DEPLOY ≠ USER ANNOUNCEMENT, solo `announceToUsers: true` genera un annuncio). Tre livelli: (1) bell del Notification Center esistente (`lib/data/notifications.ts` aggrega anche gli annunci non visti, nuovo tipo `trama_announcement` in `lib/notifications/model.ts`, isSeen dal DB reale — `announcement_receipts.seen_at`, mai un cursore client); (2) callout contestuale "✨ Nuovo" alla prima visita di una superficie pertinente (`components/nextgen/AnnouncementCallout.tsx`, caso reale implementato su `/nextgen/search`), dismesso persiste su `announcement_receipts.dismissed_at` (mai localStorage); (3) pagina storico `/nextgen/novita` (Level 3, riuso della stessa aggregazione, nessuna nuova query). Ogni annuncio con `requiredFeatureFlag` è visibile SOLO a un utente per cui quel flag risolve realmente true (stesso `resolveFeatureFlagVisibility` della capability reale) — rispetta strutturalmente internal-preview/pilot, mai un annuncio anticipato.
