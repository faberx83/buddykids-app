...
-- ─────────────────────────────────────────────
-- MIGRATION 06 (folded in) — Profilo personale esteso (genitore+gestore)
-- e registro presenze giornaliero — vedi migration_06_profilo_esteso_presenze.sql
-- per il file da eseguire STEP per STEP su un progetto già esistente.
-- ─────────────────────────────────────────────
...
-- SPRINT 8 — meccanismo di pipeline automatica: NESSUNA function/tabella
-- extra qui (prima versione, superata: vedi commento nella migration
-- standalone sprint8_beta_pipeline_migration.sql). pipeline_status viene
-- letto/scritto da un endpoint interno dell'app stessa
-- (app/api/internal/beta-pipeline/route.ts), che usa la service_role key
-- (bypassa le RLS sopra di proposito, come farebbe un admin) protetto da un
-- secret condiviso in una variabile d'ambiente Vercel — non nel database.
